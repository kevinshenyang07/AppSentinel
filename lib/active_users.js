...
const generate = (data, id, categories, axisNum) => {
  ...
  const yRange = d3.extent(ddata, d => d[option]);
...
const redraw = (data, id, categories, x, y, xAxis, svg, area, path, points, height, axisNum) => {
  //format of time data
...
export const displayActiveUsers = () => {
  ...
  const interval = setInterval(() => {
    ...
    redraw(
      activeUserData, id, categories, sca.axisOpt['x'], sca.axisOpt['y'],
